...
myApp.factory('dashboardServices', ['$http', function($http) {
    var factoryDefinitions = {
      getTodaysStats: function() {
        return $http.get('modules/dashboard/mock/todays_stats.json')
            .success(function(data) { return data; });
      },
	  getRecentNews: function() {
        return $http.get('modules/dashboard/mock/recent_news.json')
            .success(function(data, status, headers, config) { return data; });
      },
	  getLastFiveCustomers: function() {
        return $http.get('modules/dashboard/mock/customers_last_five.json')
            .success(function(data) { return data; });
      }
	}
...
//Controllers
myApp.controller('todaysStatsController', 
                 ['$scope', 
                  'dashboardServices', 
function($scope, dashboardServices) {
	dashboardServices.getTodaysStats().then(function(result){
		$scope.data = result.data;	
	});
}]);
myApp.controller('recentNewsController', 
                 ['$scope', 
                 'dashboardServices',
function($scope, dashboardServices) {
...
myApp.controller('getLastFiveCustomersController', 
                 ['$scope', 
                  'dashboardServices', 
function($scope, dashboardServices) {
